...
function delete($pdo, $tableName, $primaryKey, $id) {
    $parameters = [':id' => $id];
    query($pdo, 'DELETE FROM `' . $tableName . '`  WHERE `' . $primaryKey . '` = :id', $parameters);
}
  
function read($pdo, $tableName, $primaryKey, $id) {
    $parameters = [':id' => $id];
    $query = query($pdo, 'SELECT * FROM `' . $tableName . '`  WHERE `' . $primaryKey . '` = :id', $parameters);
    return $query->fetch();
}
  
function readAll($pdo, $tableName) {
    $query = query($pdo, 'SELECT * FROM `' . $tableName . '`  ORDER BY `board_id` DESC');
